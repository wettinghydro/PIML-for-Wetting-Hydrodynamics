...
print('iterations       = '+str(int(epochs*ntrain/batch_size)))
print('learning rate    = '+str(learning_rate))
print('scheduler step   = '+str(scheduler_step))
...
# load data
################################################################
data    = np.load(TRAIN_PATH) # x_in,y_in,θ,θs,vel_base,vel_cfd
...
cosu = my_to_torch(np.cos(u))
sinu = my_to_torch(np.sin(u))
grid = u.reshape(1, S, 1)
grid = torch.tensor(grid, dtype=torch.float)
#
#
x_in       = my_to_torch(data[0,:ntrain,::sub]).reshape(ntrain,S,1)
y_in       = my_to_torch(data[1,:ntrain,::sub]).reshape(ntrain,S,1)
centroid_x = torch.mean(x_in,1).reshape(ntrain,1,1).repeat(1,S,1)
centroid_y = torch.mean(y_in,1).reshape(ntrain,1,1).repeat(1,S,1)
rx         = x_in-centroid_x
ry         = y_in-centroid_y
r          = np.sqrt(rx**2+ry**2)
v_diff     = my_to_torch(data[5,:ntrain,::sub]-data[4,:ntrain,::sub]).reshape(ntrain,S,1)
v_base     = my_to_torch(data[4,:ntrain,::sub]).reshape(ntrain,S,1)
train_a    = torch.cat([r, v_base],dim=-1)
train_u    = v_diff
#
x_in       = my_to_torch(data[0,-ntest:,::sub]).reshape(ntest,S,1)
y_in       = my_to_torch(data[1,-ntest:,::sub]).reshape(ntest,S,1)
centroid_x = torch.mean(x_in,1).reshape(ntest,1,1).repeat(1,S,1)
centroid_y = torch.mean(y_in,1).reshape(ntest,1,1).repeat(1,S,1)
rx         = x_in-centroid_x
ry         = y_in-centroid_y
r          = np.sqrt(rx**2+ry**2)
v_diff     = my_to_torch(data[5,-ntest:,::sub]-data[4,-ntest:,::sub]).reshape(ntest,S,1)
v_base      = my_to_torch(data[4,-ntest:,::sub]).reshape(ntest,S,1)
test_a     = torch.cat([r, v_base],dim=-1)
test_u     = v_diff
#
...
model.to(device)
#
print('model parameters = {}'.format(count_params(model)))
#
...
for ep in range(epochs):
    ...
    print('===========================')
    print('epoch = '+str(ep+1)+' (wall time = '+str(t2-t1)+')')
    print('===========================')
    print('training error = '+str(separ_train_error))
    print('testing error  = '+str(separ_test_error))
    print('train mse      = '+str(train_mse),flush=True)
    #
    train_error = separ_train_error
    test_error  = separ_test_error
    if test_error<min_error:
        print('Validation error reduced. Printing model parameters...')
        torch.save({'epoch': ep, \
    ...
    print(ep+1, train_error, test_error, file=open(path_model+'_err.out', 'a'))
