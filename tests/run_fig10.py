...
#
case_name = sys.argv[1] # darts or obstacle
case   = int(sys.argv[2]) # 1 or 2

data_folder = f"../data/testing/fig10/{case_name}/case{case:1d}"
plot_folder = "plots/"
...
def het(x,y):
    if case_name == "obstacle":
        ...
        if case == 1:
            ...
            grad = np.pi/3.
            φ    = np.pi/2
        xx        = np.cos(φ)*x-np.sin(φ)*y
        yy        = np.cos(φ)*y+np.sin(φ)*x
        return np.pi/2. + \
               theta_obs*(np.tanh(b*(0.25-np.sqrt(yy**2+xx**2)))+1)/2 + \
               grad*(xx/np.sqrt(yy**2+xx**2))
    elif case_name == "darts": 
        theta_obs = np.pi/12. 
        b         = 3.
        if case == 1:
            base   = np.pi/2.
            ...
            base   = np.pi/4.
        return base + \
               theta_obs*np.tanh(b*np.sin(6.*np.arctan2(y-0.5,x-0.5)))*\
                         np.tanh(b*np.sin(4*np.pi*np.sqrt((x-0.5)**2+(y-0.5)**2)))
...
def plot_arrays(times):
    for time in times:
        ...
        tikzplot(plot_folder+f'/{case_name}_case{case:1d}_t_{float(time):06.3f}_AI.txt',np.append(x_plot,x_plot[0]),np.append(y_plot,y_plot[0]),style='pyred,thick',forget=False)
        #
        PATH = data_folder+f"/cl_t_{float(time):06.3f}.npy"
        cfd_x,cfd_y = np.load(PATH)
        plt.plot(cfd_x,cfd_y,'--k')
        tikzplot(plot_folder+f'/{case_name}_case{case:1d}_t_{float(time):06.3f}_CFD.txt',np.append(cfd_x,cfd_x[0]),np.append(cfd_y,cfd_y[0]),style='pyblue,thick,fill=pyblue,fill opacity=0.5',forget=False)
        #
    Area  = calculate_area(cfd_x,cfd_y) 
    cfd_x,cfd_y = get_cl(u,np.array([cfd_x,cfd_y]))
    error = improved_frechet(np.array([x_plot,y_plot]).T,np.array([cfd_x,cfd_y]).T,num_points=200)*np.sqrt(np.pi/(4*Area))
...
def plot_centroid():
    ...
    tikzplot(plot_folder+f'/{case_name}_case{case:1d}_centroid_t_{float(time):06.3f}_AI.txt',x_plot,y_plot,style='pyred,thick,dashed',forget=False)
        #
    tikzplot(plot_folder+f'/{case_name}_case{case:1d}_centroid_t_{float(time):06.3f}_CFD.txt',x_plot_ref,y_plot_ref,style='thick',forget=False)
...
plt.savefig(plot_folder+'/'+case_name+'_case'+str(case)+'_figure.png',bbox_inches='tight',transparent=True,format='png',dpi=300)
# plt.clf()
